	...
	void Update ()
	{
		...
		if(CurrentTeleportState == TeleportState.Teleporting)
		{
			...
			if(Time.time - TeleportTimeMarker >= TeleportFadeDuration / 2)
			{
				...
				} else
				{
					...
					offset.y = 0;
					OriginTransform.position = Disk.SelectedPoint + offset;
				}
		...
		else if(CurrentTeleportState == TeleportState.Selecting)
		{
			...
			if (shouldTeleport || shouldCancel)
			{
				...
				if (shouldTeleport && Disk.PointOnNavMesh)
				{
			...
			} else
			{
				...
				RoomBorder.enabled = Disk.PointOnNavMesh;
				RoomBorder.Transpose = Matrix4x4.TRS(Disk.SelectedPoint - offset, Quaternion.identity, Vector3.one);

				// Haptic feedback click every [HaptickClickAngleStep] degrees
				if (Disk.CurrentParabolaAngleY >= 45) // Don't click when at max degrees
					LastClickAngle = Disk.CurrentPointVector;

				float angleClickDiff = Vector3.Angle(LastClickAngle, Disk.CurrentPointVector);
				if (IsClicking && Mathf.Abs(angleClickDiff) > HapticClickAngleStep)
				{
					LastClickAngle = Disk.CurrentPointVector;
					if (Disk.PointOnNavMesh)
						device.TriggerHapticPulse();
				}

				// Trigger a stronger haptic pulse when "entering" a teleportable surface
				if (Disk.PointOnNavMesh && !IsClicking)
				{
					IsClicking = true;
					device.TriggerHapticPulse(750);
					LastClickAngle = Disk.CurrentPointVector;
				}
				else if (!Disk.PointOnNavMesh && IsClicking)
					IsClicking = false;
			}
		}
		else //CurrentTeleportState == TeleportState.None
		{
			...
			foreach (SteamVR_TrackedObject obj in Controllers)
			{
				...
				if (device.GetPressDown(SteamVR_Controller.ButtonMask.Touchpad))
				{
					...
					Disk.transform.parent = obj.transform;
					...
					Disk.ForceUpdateCurrentAngle();
					LastClickAngle = Disk.CurrentPointVector;
					IsClicking = Disk.PointOnNavMesh;
				}
			}
		}
	}
